...
void trieCheckInit (struct Trie *trie) {
	for (int i = 0; i < 26; i++) {
		if (trie->children[i] == 0 && trie->isWord[i] == 0) {
			printf("Trie at pos %c was init properly with isWord = %i.\n", 'a' + i, trie->isWord[i]);
		} else {
			printf("Trie at pos %c was not init properly with isWord = %i.\n", 'a' + i, trie->isWord[i]);
		}
	}
}
