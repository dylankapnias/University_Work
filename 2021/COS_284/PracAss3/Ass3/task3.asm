...
sin2: dq "Please input the string to encode: "                  ; Second msg prompt. 35 char long.
sout: dq "Encoded text: "                                       ; Output msg. 14 char long.
...
_start:
    ...
    mov r13, 1                                                  ; r13 = 1. Will use as shift counter.
...
while:
    ...
    mov rax, r13                                                ; Preparing to divide.
    xor rdx, rdx                                                ;
    mov r14, 2                                                  ;
    div r14                                                     ; Divide rax (r13) with 2.

    cmp byte[rsi + rcx], 32                                     ; Comparing the current char to 32 (i.e space).
    je skip                                                     ;   Goto skip if equal to.

    cmp byte[rsi + rcx], 33                                     ; Comparing the current char to 33 (i.e exclamation mark).
    je skip                                                     ;   Goto skip if equal to.

    cmp byte[rsi + rcx], 44                                     ; Comparing the current char to 44 (i.e comma).
    je skip                                                     ;   Goto skip if equal to.

    cmp byte[rsi + rcx], 46                                     ; Comparing the current char to 46 (i.e full stop).
    je skip                                                     ;   Goto skip if equal to.

    cmp byte[rsi + rcx], 63                                     ; Comparing the current char to 63 (i.e question mark).
    je skip                                                     ;   Goto skip if equal to.
...
left:
    cmp byte[rsi + rcx], 97                                     ; Comparing the current char to 97 (i.e start of lowercase).
    jl isupperL                                                 ; Goto isupper if less than.

    jmp islowerL                                                ;   Else goto islower.

right:
    cmp byte[rsi + rcx], 97                                     ; Comparing the current char to 97 (i.e start of lowercase).
    jl isupperR                                                 ; Goto isupper if less than.

    jmp islowerR                                                ;   Else goto islower.

skip:
    inc rcx                                                     ; Increase rcx to skip skip.
    jmp while                                                   ;   Goto while.
...
islowerL:
    ...
    jl wraparoundL                                              ;   Goto wraparoundL if less than.
...
islowerR:
    movsx r12, byte[rsi + rcx]                                  ; Sign extend ascii value of current char into r12
    add r12b, r8b                                               ; Add the degree of shift to the ascii val of char.

    cmp r12, 122                                                ; Compare the ascii value to 122 (i.e out of bounds for lowercase alphabet).
    jg wraparoundR                                              ;   Goto wraparoundR if greater than.

    add byte[r11 + rcx], r8b                                    ; If no wraparound, shift the current char.
    inc rcx                                                     ; Increase rcx.
    inc r13                                                     ; Increase r13.
    jmp while                                                   ;   Goto while.
...
isupperL:
    ...
    cmp r12, 65                                                 ; Compare the ascii value to 65 (i.e start of bounds for uppercase alphabet).
    jl wraparoundL                                              ;   Goto wraparoundL if less than.
...
isupperR:
    ...
    cmp r12, 90                                                 ; Compare the ascii value to 90 (i.e out of bounds for uppercase alphabet).
    jg wraparoundR                                              ;   Goto wraparoundR if greater than.
...
wraparoundR:
    xor r12, r12                                                ; r12 = 0.
    mov r12, 26                                                 ; Move 26 (length of alphabet) into r12.
    add byte[r11 + rcx], r8b                                    ; Add the shift degree to the current char.
    sub byte[r11 + rcx], r12b                                   ; Subtract the length of alphabet to get the correct character.
    inc rcx                                                     ; Increase rcx.
    inc r13                                                     ; Increase r13.
    jmp while                                                   ;   Goto while.
